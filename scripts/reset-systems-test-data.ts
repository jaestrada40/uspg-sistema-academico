...
const cycles = [
  ['CYC-2022-1', 2022, 'Primer Semestre 2022', '2022-01-10', '2022-06-04', false], ['CYC-2022-2', 2022, 'Segundo Semestre 2022', '2022-07-11', '2022-11-26', false], ['CYC-2024-2', 2024, 'Segundo Semestre 2024', '2024-07-15', '2024-11-30', false], ['CYC-2025-1', 2025, 'Primer Semestre 2025', '2025-01-13', '2025-06-07', false], ['CYC-2025-2', 2025, 'Segundo Semestre 2025', '2025-07-14', '2025-11-29', false], ['CYC-2026-1', 2026, 'Primer Semestre 2026', '2026-01-12', '2026-06-06', false], ['CYC-2026-2', 2026, 'Segundo Semestre 2026', '2026-07-13', '2026-11-28', true],
] as const;
...
await prisma.$transaction(async (tx) => {
  ...
  await tx.academicCycle.createMany({ data: cycles.map(([id, year, name, start, end, isCurrent]) => ({ id, year, name, startDate: new Date(start), endDate: new Date(end), enrollmentStartDate: new Date(start), enrollmentEndDate: new Date(start), gradeSubmissionDeadline: new Date(end), status: isCurrent ? 'En curso' : 'Finalizado', isCurrent, campusId: 'CAMPUS-CENTRAL' })) });
  await tx.classroom.createMany({ data: [{ id: 'AULA-CC-101', code: 'CC-101', building: 'Campus Central', capacity: 35, type: 'Teórica', status: 'Disponible', hasProjector: true, campusId: 'CAMPUS-CENTRAL' }, { id: 'LAB-CC-01', code: 'CC-LAB-01', building: 'Campus Central', capacity: 30, type: 'Laboratorio', status: 'Disponible', hasProjector: true, campusId: 'CAMPUS-CENTRAL' }, { id: 'AULA-ESC-101', code: 'ESC-101', building: 'Campus Escuintla', capacity: 35, type: 'Teórica', status: 'Disponible', hasProjector: true, campusId: 'CAMPUS-ESCUINTLA' }] });
  ...
  await tx.teacher.create({ data: { code: 'DOC-1042', name: 'Luis Mena', email: 'luismena@catedratico.uspg.edu.gt', phone: '+502 5512-3489', specialty: 'Ingeniería de Software', academicDegree: 'Ingeniero', assignedSectionIds: '[]', status: 'Activo', maxHoursPerWeek: 40, userId: 'USR-DOC-001', campusId: 'CAMPUS-CENTRAL' } });
