...
interface VirtualClassroomView {
  id: string; provider: string; syncStatus: string; alternateLink: string | null; enrollmentCode: string | null;
    ...
    {loading ? <p className="text-sm text-[#64748B]">Cargando aulas virtuales...</p> : (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {classrooms.map((item) => <article key={item.id} className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-xs">
          ...
          <div className="my-4 rounded-lg bg-[#F8FAFC] p-3 text-xs"><p><strong>Ciclo:</strong> {item.cycleName}</p><p className="mt-1"><strong>Estado:</strong> {item.syncStatus === 'DEMO_ACTIVE' ? 'Demostración preparada' : item.syncStatus === 'ACTIVE' ? 'Conectada' : 'Pendiente de configuración'}</p>{item.enrollmentCode && <p className="mt-1"><strong>Código:</strong> {item.enrollmentCode}</p>}{item.provider === 'DEMO_CLASSROOM' && <p className="mt-2 text-[11px] text-amber-700">Modo demostración: no existe un curso externo ni se enviaron datos a Google.</p>}</div>
          {item.alternateLink ? <a href={item.alternateLink} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 rounded-lg bg-[#800020] px-4 py-2 text-xs font-bold text-white">Entrar a Classroom <ExternalLink className="h-4 w-4" /></a> : ['ADMIN', 'REGISTRO'].includes(currentUser.role) ? <button onClick={() => sync(item.id)} className="flex w-full items-center justify-center gap-2 rounded-lg border border-[#800020] px-4 py-2 text-xs font-bold text-[#800020]"><RefreshCw className="h-4 w-4" /> Preparar aula {item.provider === 'DEMO_CLASSROOM' ? 'demo' : 'virtual'}</button> : <p className="text-center text-xs font-semibold text-amber-700">Registro Académico debe preparar esta aula.</p>}
        </article>)}
