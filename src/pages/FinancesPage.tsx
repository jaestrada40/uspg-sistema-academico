...
interface ParkingFeeScheduleRecord { id: string; periodType: string; amount: number; dueDate: string; assignedCount: number; }
...
export const FinancesPage: React.FC = () => {
  ...
  const [parkingFeeSchedules, setParkingFeeSchedules] = useState<ParkingFeeScheduleRecord[]>([]);
  ...
  const [chargePageSize, setChargePageSize] = useState(20);

  const loadFinances = useCallback(async () => {
    setLoading(true);
    const query = ['ADMIN', 'FINANZAS'].includes(currentUser.role) && studentCarnet ? `?studentCarnet=${encodeURIComponent(studentCarnet)}` : '';
    try {
  ...
  useEffect(() => { if (currentUser.role === 'ESTUDIANTE') fetch('/api/finances/card-payment-status').then((response) => response.ok ? response.json() : { demoAvailable: false, stripeAvailable: false }).then((result) => { setCardDemoAvailable(Boolean(result.demoAvailable)); setStripeAvailable(Boolean(result.stripeAvailable)); }).catch(() => { setCardDemoAvailable(false); setStripeAvailable(false); }); }, [currentUser.role]);

  const loadStatement = useCallback(async () => {
    const params = new URLSearchParams({ from: statementDates.from, to: statementDates.to });
    if (['ADMIN', 'FINANZAS'].includes(currentUser.role)) params.set('studentCarnet', studentCarnet);
  ...
  const statementPdfUrl = `/api/finances/statement.pdf?${new URLSearchParams({ ...(['ADMIN', 'FINANZAS'].includes(currentUser.role) ? { studentCarnet } : {}), from: statementDates.from, to: statementDates.to })}`;

  const loadTransferProofs = useCallback(async () => { const query = ['ADMIN', 'FINANZAS'].includes(currentUser.role) && studentCarnet ? `?studentCarnet=${encodeURIComponent(studentCarnet)}` : ''; const response = await fetch(`/api/finances/transfer-proofs${query}`); if (response.ok) setTransferProofs(await response.json()); }, [currentUser.role, studentCarnet]);
  ...
  const loadParkingFeeSchedules = useCallback(async () => {
    if (!['ADMIN', 'FINANZAS'].includes(currentUser.role)) return;
    const response = await fetch('/api/parking/fee-schedules');
    if (response.ok) setParkingFeeSchedules(await response.json());
  }, [currentUser.role]);

  useEffect(() => { loadParkingFeeSchedules(); }, [loadParkingFeeSchedules]);

  useEffect(() => { if (['ADMIN', 'FINANZAS'].includes(currentUser.role)) fetch('/api/academic-structure').then((response) => response.ok ? response.json() : Promise.reject()).then((result) => setAcademicStructure({ campuses: result.campuses.filter((campus: { status: string }) => campus.status === 'Activo'), plans: result.plans.filter((plan: { status: string }) => plan.status === 'Activo') })).catch(() => showToast('No se pudo cargar campus y planes', 'error')); }, [currentUser.role, showToast]);
  ...
  const startStripeCheckout = async () => { const charge = charges.find((item) => item.balance > 0); if (!charge) return showToast('No tienes cargos pendientes para pagar', 'warning'); const response = await fetch('/api/finances/stripe/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chargeId: charge.id }) }); const result = await response.json(); if (!response.ok) return showToast(result.message, 'error'); window.location.assign(result.checkoutUrl); };
    ...
    <RoleGuard allowedRoles={['ADMIN', 'FINANZAS', 'ESTUDIANTE']}>
      <div className="space-y-6">
        <PageHeader
          ...
          actions={<div className="flex flex-wrap gap-2"><a href={statementPdfUrl} className="flex items-center gap-2 rounded-lg border border-[#E2E8F0] bg-white px-3 py-2 text-xs font-bold text-[#333333]"><Download className="h-4 w-4 text-[#800020]" />Estado de Cuenta PDF</a>{['ADMIN', 'FINANZAS'].includes(currentUser.role) && <><button onClick={() => setSpecialForm('MORA')} className="flex items-center gap-2 rounded-lg border border-[#E2E8F0] bg-white px-3 py-2 text-xs font-bold"><AlertTriangle className="h-4 w-4 text-amber-600" />Mora</button><button onClick={() => setSpecialForm('CONVENIO')} className="flex items-center gap-2 rounded-lg border border-[#E2E8F0] bg-white px-3 py-2 text-xs font-bold"><Handshake className="h-4 w-4 text-[#17A2B8]" />Convenio</button><button onClick={() => setShowScheduleForm((value) => !value)} className="flex items-center gap-2 rounded-lg bg-[#800020] px-4 py-2 text-xs font-bold text-white"><CalendarDays className="h-4 w-4" />Calendario de cuotas</button><button onClick={() => setShowChargeForm((value) => !value)} className="flex items-center gap-2 rounded-lg border border-[#800020] bg-white px-4 py-2 text-xs font-bold text-[#800020]"><Plus className="h-4 w-4" />Cargo único</button></>}</div>}
        />

        {currentUser.role === 'ESTUDIANTE' && (cardDemoAvailable || stripeAvailable) && <section className="overflow-hidden rounded-xl border border-[#800020]/20 bg-white shadow-xs"><div className="flex flex-col justify-between gap-4 bg-gradient-to-r from-[#800020] to-[#A61B3D] p-5 text-white sm:flex-row sm:items-center"><div className="flex items-start gap-3"><div className="rounded-xl bg-white/15 p-3"><CreditCard className="h-6 w-6" /></div><div><div className="flex items-center gap-2"><h2 className="font-extrabold">Pago en línea con tarjeta</h2><span className="rounded-full bg-amber-300 px-2 py-0.5 text-[9px] font-black text-amber-950">{stripeAvailable ? 'STRIPE' : 'DEMOSTRACIÓN'}</span></div><p className="mt-1 max-w-2xl text-xs text-white/85">{stripeAvailable ? 'Serás redirigido a Stripe para completar el pago de forma segura.' : 'Prueba cómo funcionará el pago con crédito o débito.'}</p></div></div><button onClick={stripeAvailable ? startStripeCheckout : openCardDemo} className="whitespace-nowrap rounded-lg bg-white px-5 py-2.5 text-xs font-extrabold text-[#800020] shadow-sm">{stripeAvailable ? 'Pagar con Stripe' : 'Probar pago con tarjeta'}</button></div>{cardDemoAvailable && cardDemoOpen && <div className="p-5"><form onSubmit={simulateCardPayment} className="grid gap-4 lg:grid-cols-2"><label className="text-xs font-bold lg:col-span-2">Cargo a pagar<select required value={cardChargeId} onChange={(event) => setCardChargeId(event.target.value)} className="mt-1 w-full rounded-lg border px-3 py-2.5 font-normal">{charges.filter((charge) => charge.balance > 0).map((charge) => <option key={charge.id} value={charge.id}>{charge.concept} · {money(charge.balance)}</option>)}</select></label><label className="text-xs font-bold lg:col-span-2">Nombre del titular<input required minLength={3} value={cardForm.cardholder} onChange={(event) => setCardForm({ ...cardForm, cardholder: event.target.value })} className="mt-1 w-full rounded-lg border px-3 py-2.5 font-normal" /></label><button disabled={cardProcessing} className="lg:col-span-2 rounded-lg bg-[#800020] px-5 py-3 text-xs font-extrabold text-white">Simular pago seguro</button></form></div>}</section>}
        ...
        {showScheduleForm && ['ADMIN', 'FINANZAS'].includes(currentUser.role) && (
        ...
        {['ADMIN', 'FINANZAS'].includes(currentUser.role) && (
          <div className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-xs">
            ...
            {parkingFeeSchedules.length > 0 && (
              <div className="mt-4 grid gap-3 lg:grid-cols-2">{parkingFeeSchedules.slice(0, 8).map((schedule) => <div key={schedule.id} className="flex items-center justify-between rounded-lg bg-[#F8FAFC] p-3"><div><p className="text-xs font-bold text-[#333333]">{schedule.periodType}</p><p className="text-[11px] text-[#64748B]">Vence {new Date(schedule.dueDate).toLocaleDateString('es-GT')} · {schedule.assignedCount} vehículos</p></div><span className="text-sm font-extrabold text-[#800020]">{money(schedule.amount)}</span></div>)}</div>
            )}
          </div>
        )}

        {showChargeForm && ['ADMIN', 'FINANZAS'].includes(currentUser.role) && (
        ...
        {specialForm && ['ADMIN', 'FINANZAS'].includes(currentUser.role) && (
        ...
        {['ADMIN', 'FINANZAS'].includes(currentUser.role) && careerFees.length > 0 && (
          <div className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-xs">
        ...
        {adjustmentCharge && ['ADMIN', 'FINANZAS'].includes(currentUser.role) && (
