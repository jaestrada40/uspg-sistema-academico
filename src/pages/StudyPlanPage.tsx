...
export const StudyPlanPage: React.FC = () => {
  ...
  return <RoleGuard allowedRoles={['ADMIN', 'ESTUDIANTE']}><div className="space-y-6">
    ...
    {currentUser.role === 'ADMIN' && <div className="rounded-xl border bg-white p-5"><label className="mb-1 block text-xs font-bold uppercase text-[#64748B]">Consultar plan del estudiante</label><select value={studentCarnet} onChange={(event) => setStudentCarnet(event.target.value)} className="w-full max-w-xl rounded-lg border bg-[#F8FAFC] px-3 py-2 text-sm font-semibold">{students.map((student) => <option key={student.carnet} value={student.carnet}>{student.carnet} - {student.name}</option>)}</select></div>}
    {loading ? <div className="rounded-xl border bg-white p-12 text-center text-sm text-[#64748B]">Cargando plan de estudios...</div> : data && <>
      ...
      <div className="overflow-x-auto rounded-xl border bg-[#F8FAFC] p-4 shadow-inner"><div className="grid min-w-[900px] gap-4" style={{ gridTemplateColumns: `repeat(${Math.min(data.curriculum.semesters, 5)}, minmax(260px, 1fr))` }}>
