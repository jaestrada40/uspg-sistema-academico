            ...
            <tbody className="divide-y">
              {paginated.map((user) => (
                <tr key={user.id} className={!user.active ? 'bg-slate-50 opacity-60' : ''}>
                  ...
                  <td className="p-3"><div className="flex flex-col items-start gap-1"><span className={`whitespace-nowrap rounded-full px-2 py-1 text-[10px] font-bold ${user.mfaEnabled ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-700'}`}>MFA {user.mfaEnabled ? 'activo' : 'inactivo'}</span>{user.mustChangePassword && <span className="whitespace-nowrap rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-800">Cambio pendiente</span>}</div></td>
                  <td className="p-3"><div className="flex flex-col items-end gap-1.5">
