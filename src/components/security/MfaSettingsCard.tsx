  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-6 shadow-xs">
        ...
        {!status.enabled && !setup && <div className="mt-5 grid gap-3 border-t pt-5 sm:grid-cols-[1fr_auto]"><input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Confirma tu contraseña actual" className="rounded-lg border px-3 py-2 text-xs" /><button disabled={busy || !currentPassword} onClick={startSetup} className="rounded-lg bg-[#800020] px-4 py-2 text-xs font-bold text-white disabled:opacity-50">Configurar MFA</button></div>}

        {setup && <div className="mt-5 grid gap-5 border-t pt-5 md:grid-cols-[280px_1fr]"><img src={setup.qrDataUrl} alt="Código QR para configurar MFA" className="w-full max-w-[280px] rounded-xl border" /><div className="space-y-4 text-xs"><div><p className="font-bold">1. Escanea el QR</p><p className="mt-1 text-[#64748B]">Usa Google Authenticator, Microsoft Authenticator, Authy u otra aplicación TOTP.</p></div><div><p className="font-bold">2. Clave manual</p><code className="mt-1 block break-all rounded-lg bg-slate-100 p-3 font-mono">{setup.secret}</code></div><div><p className="mb-1 font-bold">3. Confirma el código actual</p><div className="flex gap-2"><input autoComplete="one-time-code" value={code} onChange={(e) => setCode(e.target.value)} placeholder="000000" className="min-w-0 flex-1 rounded-lg border px-3 py-2 text-center font-mono font-bold tracking-widest" /><button disabled={busy || !/^\d{6}$/.test(code)} onClick={enable} className="rounded-lg bg-green-600 px-4 py-2 font-bold text-white disabled:opacity-50">Activar</button></div></div></div></div>}

        {status.enabled && <div className="mt-5 grid gap-3 border-t pt-5 sm:grid-cols-2"><input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="Contraseña actual" className="rounded-lg border px-3 py-2 text-xs" /><input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="Código MFA o recuperación" className="rounded-lg border px-3 py-2 text-xs" /><button disabled={busy || !currentPassword || !code} onClick={regenerate} className="flex items-center justify-center gap-2 rounded-lg border px-4 py-2 text-xs font-bold"><RefreshCw className="h-4 w-4" />Renovar códigos</button><button disabled={busy || status.required || !currentPassword || !code} onClick={disable} className="rounded-lg border border-red-200 px-4 py-2 text-xs font-bold text-red-700 disabled:opacity-40">Desactivar MFA</button></div>}
      </div>
