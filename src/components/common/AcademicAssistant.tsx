...
export const AcademicAssistant: React.FC = () => {
  ...
  const clearConversation = async () => { if (!conversationId || !window.confirm('¿Borrar esta conversación?')) return; const response = await fetch(`/api/assistant/conversations/${conversationId}`, { method: 'DELETE' }); if (response.ok) { setConversationId(null); setMessages([{ from: 'bot', text: 'Historial borrado. Puedes iniciar una nueva consulta.' }]); } };
