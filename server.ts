...
// Timeouts explícitos: por defecto nodemailer espera hasta 2 min para conectar. Si el
// host de producción no alcanza al servidor SMTP, ese cuelgue se traslada a quien hizo
// la petición (p. ej. el agente de WhatsApp, que corta mucho antes). Con estos límites
// el envío falla rápido y el correo queda en la cola (email_outbox) para reintento.
const mailTransport = process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS ? nodemailer.createTransport({ host: process.env.SMTP_HOST, port: smtpPort, secure: smtpPort === 465, auth: { user: process.env.SMTP_USER, pass: process.env.SMTP_PASS }, connectionTimeout: 10_000, greetingTimeout: 8_000, socketTimeout: 15_000 }) : null;
