...
// ── Auth helpers ──────────────────────────────────────────────────────────────
const publicUser = (user: { id: string; name: string; email: string; role: string; avatar: string | null; carnetOrCode: string | null; phone: string | null; department: string | null; mustChangePassword: boolean; mfaEnabled: boolean }, requiredRoles: string[] = defaultMfaRequiredRoles) => ({ id: user.id, name: user.name, email: user.email, role: user.role, avatar: user.avatar || undefined, carnetOrCode: user.carnetOrCode || undefined, phone: user.phone || undefined, department: user.department || undefined, mustChangePassword: user.mustChangePassword, mfaEnabled: user.mfaEnabled, mfaEnrollmentRequired: requiredRoles.includes(user.role) && !user.mfaEnabled });
