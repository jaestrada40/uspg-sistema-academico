...
const emailHtml = (title: string, message: string, link?: string, _hasLogo = false) => {
  ...
  const logo = '<div style="display:inline-flex;align-items:center;justify-content:center;width:54px;height:54px;border-radius:12px;background:#8b0028;color:#fff;font-size:20px;font-weight:800;letter-spacing:-1px;margin-bottom:14px">USPG</div>';
...
const deliverOutboxEmail = async (outboxId: string) => {
  ...
  try {
    ...
    const logoAttachment = institution?.logoDataUrl?.match(/^data:(image\/(?:png|jpeg|webp));base64,(.+)$/);
