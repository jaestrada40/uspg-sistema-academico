...
const emailHtml = (title: string, message: string, link?: string, hasLogo = false) => {
  ...
  const logo = hasLogo ? '<img src="cid:uspg-logo" alt="USPG" width="74" style="display:block;width:74px;height:74px;object-fit:contain;margin-bottom:14px;background:#fff;border-radius:10px;padding:4px">' : '<div style="display:inline-flex;align-items:center;justify-content:center;width:54px;height:54px;border-radius:12px;background:#8b0028;color:#fff;font-size:20px;font-weight:800;letter-spacing:-1px;margin-bottom:14px">USPG</div>';
...
const deliverOutboxEmail = async (outboxId: string) => {
  ...
  try {
    ...
    const fallbackLogo = (() => { try { return `data:image/webp;base64,${readFileSync(path.join(rootDir, 'public/logo-uspg.webp')).toString('base64')}`; } catch { return null; } })();
